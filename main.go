...
import (
	...
	"github.com/ilakianpuvanendra/ngxstat/ngxstat"
)
...
// overridable with NGXSTAT_LOGS_PATH env var
const DEFAULT_PATH_PATTERN = "/var/log/nginx/access.log*"
const DEFAULT_DB_PATH = "./ngxstat.db"
...
func main() {
	...
	if os.Getenv("NGXSTAT_DEBUG") == "" {
		log.Default().SetOutput(io.Discard)
	}

	dbPath := DEFAULT_DB_PATH
	if envPath := os.Getenv("NGXSTAT_DB"); envPath != "" {
		dbPath = envPath
	}

	logPathPattern := DEFAULT_PATH_PATTERN
	if envLogsPath := os.Getenv("NGXSTAT_LOGS_PATH"); envLogsPath != "" {
		logPathPattern = envLogsPath
	}

	logFormat := DEFAULT_LOG_FORMAT
	if envLogFormat := os.Getenv("NGXSTAT_LOG_FORMAT"); envLogFormat != "" {
		logFormat = envLogFormat
	}

	parser := ngxstat.NewParser(logFormat)

	ctx, spec := querySpecFromCLI()
	dbs, err := ngxstat.InitDB(dbPath, parser.Fields)
...
func querySpecFromCLI() (*kong.Context, *ngxstat.RequestCountSpec) {
	// Parse query spec first, i.e. don't bother with db updates if the command is invalid
	fieldNames := make([]string, 0, len(ngxstat.CLI_NAME_TO_FIELD))
	for k := range ngxstat.CLI_NAME_TO_FIELD {
	...
	ctx := kong.Parse(
		&cli,
		kong.Description("ngxstat prints request counts from nginx access.logs based on a command-line query"),
		kong.UsageOnError(),
		kong.Vars{
			"version": "ngxstat v1.0.0",
	...
	for i, field := range cli.Fields {
		columns[i] = ngxstat.CLI_NAME_TO_FIELD[field].ColumnName
	}

	whereConditions, err := resolveWhereConditions(cli.Where)
	ctx.FatalIfErrorf(err)

	spec := &ngxstat.RequestCountSpec{
...
func resolveWhereConditions(clauses []string) (map[string][]string, error) {
	...
	for _, clause := range clauses {
		...
		if field, found := ngxstat.CLI_NAME_TO_FIELD[keyvalue[0]]; found {
...
func loadLogs(parser *ngxstat.LogParser, logPathPattern string, dbs *ngxstat.DBSession) error {
