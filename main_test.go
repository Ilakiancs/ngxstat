...
func TestUpdatedLog(t *testing.T) {
	...
	dbFile, err := os.CreateTemp("", "ngxstat.db")
	assertEqual(t, err, nil)
	defer os.Remove(dbFile.Name())

	parser := ngxstat.NewParser(DEFAULT_LOG_FORMAT)
	dbs, err := ngxstat.InitDB(dbFile.Name(), parser.Fields)
	assertEqual(t, err, nil)
	defer dbs.Close()

	os.Args = []string{"ngxstat"}
...
func runCommand(t *testing.T, format string, logs string, cliArgs []string) ([]string, [][]string) {
	// write the logs to a temp file, and point the NGXSTAT_LOGS_PATH env to it
	logFile, err := os.CreateTemp("", "access.log")
	assertEqual(t, err, nil)
	defer os.Remove(logFile.Name())
	...
	os.Args = append([]string{"ngxstat"}, cliArgs...)
	_, spec := querySpecFromCLI()

	parser := ngxstat.NewParser(format)
	dbs, err := ngxstat.InitDB(dbFile.Name(), parser.Fields)
	assertEqual(t, err, nil)
	defer dbs.Close()
